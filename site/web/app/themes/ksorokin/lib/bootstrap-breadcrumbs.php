...
class Bootstrap_Breadcrumb_Trail extends Breadcrumb_Trail {
	...
	public function trail() {
		...
		if ( !empty( $this->items ) && is_array( $this->items ) ) {
			...
			$breadcrumb = "\n\t\t" . '<' . tag_escape( $this->args['container'] ) . ' class="breadcrumb" >';

			/* If $before was set, wrap it in a container. */
			$breadcrumb .= ( !empty( $this->args['before'] ) ? "\n\t\t\t" . '<li class="breadcrumb-item">' . $this->args['before'] . '</li> ' . "\n\t\t\t" : '' );
			...
			foreach ( $this->items as $item ) {
				...
				$temp_array[] = '<li class="breadcrumb-item" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">' . $item . '</li>';
			...
			if ( !empty( $first_item ) )
				array_unshift( $this->items, '<li itemscope itemtype="http://data-vocabulary.org/Breadcrumb" class="breadcrumb-item"><span class="breadcrumb-home"></span> ' . $first_item . '</li>' );

			/* Adds the 'trail-end' class around last item. */
			if ( is_front_page() ) {

				array_push( $this->items, '<li class="breadcrumb-item active">' . $last_item . '</li>' );
